...
/* ============================================================================
 * Rino Jose: Converting this from header-only to a C file that works with the
 * existing base64 API from proto1.
 */
...
/* ============================================================================
 * (Rino Jose): Here are my wrapper/adapter functions
 */
static int calcDecodeLength(const char* b64input)
{
...
{
        int flen;
        *dst = base64((const void *)src, strlen(src), &flen);
        return 0;
}

int base64_decode(char **dst, const char *b64_src)
{
        int flen = calcDecodeLength(b64_src) + 1;
        *dst = unbase64(b64_src, strlen(b64_src), &flen);
        (*dst)[flen] = '\0';
        return 0;
}
